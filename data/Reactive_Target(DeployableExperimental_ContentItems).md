...
Once the Reactive Target is placed on the ground, it is propped up. It simulates the damage taken by a player. When you shoot at it or hit the target side, it will take simulated damage and sparks will fly out from the target. The target sign will drop when it has taken the simulated damage needed to kill a player, 100 (theoretical) health. You can prop it up again by mousing over the target and pressing the Use button (default "E" key).
Hitting the bull's eye of the target will simulate a headshot, dealing more theoretical damage to the target. Hitting anywhere other than the bull's eye will simulate torso damage.
The target itself has 250 health, and with every hit the target goes down by 1 health. The target can be repaired with the Hammer.
# Skins
...
The target sign itself doesn't need to take 100 damage to fall over. For example, shooting the bull's eye 5 times with an Assault Rifle will not do 100 damage to the target sign, but the target will simulate a player taking 5 headshots of damage from the Assault Rifle, which would typically result in a kill.
Hitting the bullseye results in more sparks flying than the typical amount from anywhere other than the bullseye.
Items
Ammunition
