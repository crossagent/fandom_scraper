...
The Silencer can be crafted with:
5 High Quality Metal
It takes 120 seconds (2 minutes) to craft the Silencer.
# Damage reduction

The silencer reduces the damage of an Assault Rifle by a fourth and the bleeding by a fourth. 
For example instead of the approximate 44 damage the Assault Rifle does it is instead reduced to 33 damage. This makes it a 4 shot kill instead of a 2-3 shot kill. It is safe to assume this applies to all other guns as well, as of the recent update, the silencer now does 25% damage reduce instead of the old 30%.
It also removes the tracer tracking feature of the rifle, so that you will not be able to "see" the trajectory of your shots.
# Durability

The silencer, unlike other weapon attachments, is subject to durability loss the same way weapons are. 
On average the silencer will manage between 300 - 320 rounds(regardless of ammo type) before breaking. Once broken the silencer prevents further operation of the weapon, unless detached prior to firing the gun. A broken silencer's health bar can be replenished by repairing it on a Repair Bench. It costs 4 High Quality Metal to repair the silencer and doing so will incur the standard halved-durability repair penalty associated with the Repair Bench. (as of most recent update the durability of the silencer has been doubled meaning that it takes twice as long before it breaks, the old durabilty was 150 - 160 rounds before breaking). 
Items
Ammunition
