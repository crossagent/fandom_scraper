...
The map can be equipped on the hot bar. It can be accessed at any time by holding the "G" key by default. Or, in inventory, right click on it. Once the map is opened, the player can right click and drag to see different areas of the map, scroll in and out to zoom and also see a drawing box at the bottom right hand corner of the window. The player's location is shown as a dot.
Select size, shape, opacity, colour in that order from top to bottom. Then left click and drag to draw on the map.
To remove drawings change the shape to the square with the cross and then left click and drag over what you want to remove.
# Notes
